...
This file is generated by `pi-odoo-skill-manager`.
...
def scan_content_hygiene(root: Path) -> tuple[str, str, str]:
    ...
    for path in root.rglob("*"):
        ...
        if path.name == ".envrc.local":
            continue
        if path.name == "pi_odoo_skill_manager.py":
            continue
...
def doctor_recommendations(results: list[tuple[str, str, str]], project_dir: Path) -> list[str]:
    ...
    for name, status, message in results:
        ...
        if name == "tool:python3":
            ...
            add(f"Run setup: pi-odoo-skill-manager wizard \"{project_dir}\"")
        elif name == "link:.pi/skill-manager":
            add(f"Recreate project entrypoint: pi-odoo-skill-manager wizard \"{project_dir}\"")
        elif name == "skills:collisions":
            add(f"Clean collisions: pi-odoo-skill-manager cleanup \"{project_dir}\" --all, then rerun setup.")
        elif name == "skills:invalid-artifacts":
            add(f"Archive invalid artifacts via setup: pi-odoo-skill-manager wizard \"{project_dir}\"")
...
def setup_project_quick(root: Path, project_dir: Path) -> list[str]:
    ...
    target = root / "pi_odoo_skill_manager.py"
    ...
    def _app(stdscr: curses.window) -> None:
        ...
        while True:
            ...
            if key in (ord("\n"), curses.KEY_ENTER, ord(" ")):
                ...
                curses.endwin()
                try:
                    subprocess.call([sys.executable, str(root / "pi_odoo_skill_manager.py"), "doctor", str(project_dir)])
