...
def wizard(
    ...
    click.echo(f"Installed skill manager setup into: {project_dir}")
    click.echo("\n[skills]")
...
def cleanup_cmd(project_repo_path: Path | None, remove_local_exclude: bool, remove_all: bool, yes: bool) -> None:
    ...
    if interactive:
        remove_skills = click.confirm("Remove installed shared skills from project (.pi/skills/shared-devkit)?", default=True)
        ...
        remove_devkit_backups = click.confirm("Remove skill-manager backup archives (.pi/_devkit-backups, .pi/_legacy-tools-disabled, old .pi/skills/_disabled-by-devkit)?", default=True)
